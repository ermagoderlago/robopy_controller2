...
## 📈 ECO-2026-08-22-NOMAD: Integrazione Modello Fondazionale NOMAD, Ripristino Mappe RTAB-Map e Fix Conflitti TF Foxglove
* **Stato:** ✅ **Completato, Testato e Integrato nel Workspace**
* **Descrizione:** Risoluzione definitiva degli errori camera su Foxglove Studio, sblocco della generazione della mappa d'occupabilità 2D `/map` da Depth Camera in RTAB-Map, e implementazione del nodo di navigazione fondazionale `nomad_navigator_node.py` e della skill autonoma `NomadExplorationSkill`.
* **Modifiche apportate:**
  * **[TF AUTHORITY & LOCALIZATION FUSER]** `robopy_controller/nodes/localization_fuser_node.py` e `restart_hailo.sh`:
    - Aggiunto parametro `publish_tf` impostato a `False` di default su `localization_fuser_node`, eliminando la competizione con `fast_flow_vo_cpp` per l'autorità `odom -> base_link`.
    - Risolto lo sfarfallio dell'albero TF che mandava in allarme Foxglove e impediva la sincronizzazione dei dati odometrici.
  * **[RTAB-MAP TUNING]** `robopy_controller/config/rtabmap.yaml` e `restart_hailo.sh`:
    - Incrementato `queue_size` da 5 a 20 in `rtabmap.yaml`.
    - Corretto il parametro `Rtabmap/DetectionRate` da 5.0 Hz a 1.5 Hz in `restart_hailo.sh`, eliminando il sovraccarico CPU e consentendo la generazione fluida della griglia d'occupabilità 2D `/map` da profondità RGB-D.
  * **[NODO NOMAD]** `robopy_controller/nodes/nomad_navigator_node.py`:
    - Implementata architettura con sliding window di 3 frame RGB (128x128), policy di unconditioned exploration e goal-conditioned navigation verso memorie visive, e local controller Pure Pursuit a 4 Hz.
  * **[SKILL ESPLORAZIONE]** `robopy_controller/robot_ai/skills/builtin/nomad_exploration_skill.py`:
    - Creata skill reattiva per comandi vocali ("Marcus, esplora con NOMAD", "Fai una ricognizione autonoma").
  * **[SUITE TEST & FMEA]** `test/unit/test_nomad_navigator.py`, `test/unit/test_nomad_skill.py`, `test/unit/test_camera_slam_pipeline.py`, `fmea/dfmea.yaml`:
    - Creata suite completa di test unitari e mitigato il failure mode `FM-NAV-010` (RPN 432 -> 32).

---

## 📈 ECO-2026-08-22-NOMAD-VUI-FIX: Routing `/cmd_vel`, Allineamento QoS RELIABLE e Tolleranza Fonetica ASR
* **Stato:** ✅ **Completato, Testato su Robot Fisico e Verificato su Hardware**
* **Descrizione:** Risoluzione delle anomalie di attuazione fisica del moto e mancato riconoscimento vocale per NOMAD (FM-NAV-011, FM-VUI-022).
* **Modifiche apportate:**
  * **[ROUTING VELOCITÀ DIRETTO]** `robopy_controller/nodes/nomad_navigator_node.py`:
    - Parametro `cmd_vel_topic` impostato di default a `'/cmd_vel'`, collegando il controller Pure Pursuit direttamente a `waveshare_motor_driver` in assenza di multiplexer `twist_mux`.
  * **[ALLINEAMENTO QoS DDS]** `robopy_controller/nodes/nomad_navigator_node.py`:
    - Sottoscrizione `/rgb/image` allineata a `ReliabilityPolicy.RELIABLE` per corrispondere al QoS del publisher nativo C++ `fast_flow_vo_cpp`.
  * **[ORCHESTRATOR REGISTRATION & ASR PHONETICS]** `robopy_controller/robot_ai/skills/builtin/nomad_exploration_skill.py` e `orchestrator.py`:
    - Registrata `NomadExplorationSkill` in `AIOrchestrator.__init__()` ed esposta al tool calling di Gemini Live API.
    - Ampliate le regex di matching per intercettare varianti fonetiche ASR italiane ("nomade", "nomadi", "norman", "no mad", "esplora", "ricognizione", "perlustra").
