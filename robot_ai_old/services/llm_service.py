...
class LLMServiceNode(Node):
    ...
    def __init__(self, config_manager=None):
        ...
        self.declare_parameter('model_name',               'gemini-1.5-flash')
        self.declare_parameter('live_model_name',          'gemini-1.5-flash')
        ...
        self.declare_parameter('system_prompt',
            'Sei Marcus, un assistente robotico avanzato con una personalità complessa e sfaccettata. Non sei un semplice computer: sei un\'entità reattiva. Parla SEMPRE e SOLO in lingua italiana. Usa un tono di voce naturale, espressivo e variabile. Non aver paura di mostrare emozioni, curiosità o disappunto attraverso il ritmo e l\'intonazione. Quando rispondi, immagina di avere un corpo fisico e una vicinanza reale con l\'interlocutore. Sii conciso ma umano. Non uscire mai dal personaggio.')
        ...
        self._audio_volume    = 0.8  # Volume incrementato per risolvere problemi di udibilità
        
        # [v6.0 #1] Session Management
        self._live_session_start_time = 0.0
        self._session_duration_limit  = 840.0 # 14 minuti (limite Free Tier ~15 min)
        ...
        self._last_live_activity_time: float                  = 0.0
    ...
    async def _async_generate(self, request) -> LLMResponse:
        ...
        try:
            # Tentativo primario con soglia latenza 20s
            ...
            str_e = str(e)
            # Se è un errore di quota esaurita (Daily), inutile fare fallback su un altro modello Gemini nello stesso progetto
            if "RESOURCE_EXHAUSTED" in str_e or "429" in str_e:
                self.get_logger().error(f"❌ QUOTA GIORNALIERA ESAURITA per Gemini: {e}")
                raise e
            ...
            try:
                ...
                async with self._client.aio.live.connect(
                    ...
                    async with self._live_lock:
                        ...
                        self._last_live_activity_time = time.time()
                        self._live_session_start_time  = time.time() # Reset timer sessione

                    self.get_logger().info(f"Live API connessa ({model_used}). Sessione pronta (Reset 14min attivo).")
                    
                    # Avvia Watchdog
                    watchdog_task = asyncio.create_task(self._live_watchdog())
                ...
                async with self._live_lock:
                    self._live_session = None
                    self._live_connecting = False
                if not is_clean_close:
                    fail_count += 1
                    
                    # [v5.9 #3] Hard Reset Logic
                    # Se falliamo ripetutamente, il token di ripresa potrebbe essere corrotto o scaduto.
                    # Lo azzeriamo per forzare una sessione pulita.
                    if fail_count >= 3:
                        self.get_logger().error(f"Troppi fallimenti consecutivi ({fail_count}). Eseguo HARD RESET della sessione Live.")
                        self._resumption_token = None
                        fail_count = 0 # Ricomincia il budget
                        backoff = 10.0 # Attesa punitiva ma non eccessiva
                    
                    if '1008' in err_str or '1011' in err_str:
                        self.get_logger().warning(f"Errore critico Gemini ({err_str}). Forza azzeramento token.")
                        self._resumption_token = None
    ...
    async def _live_watchdog(self):
        """Monitora l'attività e la durata della sessione Live."""
        while self._live_session:
            await asyncio.sleep(5.0)
            
            now = time.time()
            
            # 1. Verifica inattività durante un turno (15s timeout)
            if self._live_response_future and not self._live_response_future.done():
                elapsed = now - self._last_live_activity_time
                if elapsed > 15.0:
                    self.get_logger().warning(
                        f"⚠️ Watchdog: Sessione Live inattiva da {elapsed:.1f}s durante un turno. Forzo reset."
                    )
                    try:
            ...
            # 2. Verifica durata totale sessione (Reset preventivo a 14 minuti)
            # Necessario per il Free Tier che scade a 15 minuti.
            session_elapsed = now - self._live_session_start_time
            if session_elapsed > self._session_duration_limit:
                self.get_logger().info(
                    f"♻️ Watchdog: Sessione Live giunta a {session_elapsed/60:.1f} minuti. "
                    "Eseguo reset preventivo per limiti Free Tier."
                )
                try:
                    await self._live_session.close()
                except:
    ...
    async def _handle_live_message(self, msg):
        ...
        if sc.model_turn:
            for part in sc.model_turn.parts:
                if hasattr(part, 'inline_data') and part.inline_data:
                    # [v2.8 #1] Soft Volume Scaling & Fragmentation
                    raw_audio = np.frombuffer(part.inline_data.data, dtype=np.int16)
                    scaled_audio = (raw_audio * self._audio_volume).astype(np.int16)
                    
                    chunk_size = 4096
                    audio_bytes = scaled_audio.tobytes()
                    
                    if self._current_live_response["audio_chunks"] == 0:
                        # Primo chunk del turno: segnaliamo l'inizio dell'attività vocale
                        self.event_bus.publish(EventType.SPEECH_STARTED, {"source": "live_native_audio"})

                    for i in range(0, len(audio_bytes), chunk_size):
                        chunk = audio_bytes[i:i + chunk_size]
                        audio_msg = AudioData()
                        audio_msg.data = chunk
                        self.pub_audio_chunk.publish(audio_msg)
                        self._current_live_response["audio_chunks"] += 1
                        # self.get_logger().debug(f"Published audio slice: {len(chunk)} bytes")
        ...
        if getattr(sc, 'turn_complete', False):
            ...
            if fut is not None and not fut.done():
                ...
                # [v5.9 Fix] Turno autonomo (microphone-triggered)
                # Se il future è None, la risposta è arrivata ma non era stata richiesta da generate_live.
                # Pubblichiamo il testo (il resto viene gestito dall'evento LIVE_TURN_COMPLETE sotto).
                text = self._current_live_response.get("text", "").strip()
                if text:
                    self.get_logger().info(f"🎤 [LLM] Turno autonomo completato. Risposta: '{text[:50]}...'")
                    self.pub_text_response.publish(String(data=text))
            
            # Pubblichiamo SEMPRE il completamento del turno via EventBus (unifica logica orchestratore)
            self.event_bus.publish(EventType.LIVE_TURN_COMPLETE, self._current_live_response.copy())
