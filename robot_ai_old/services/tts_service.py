...
class TTSService:
    ...
    async def _synthesize(self, text: str, language: str = None) -> str:
        ...
        if self._client:
            ...
            try:
                ...
                # Strip WAV header (44 bytes) to get raw PCM 16kHz s16le mono
    ...
    async def _play_audio(self, filename: str) -> None:
        ...
        try:
            ...
            # 16000 Hz * 16-bit (2 bytes) * 1 channel = 32000 bytes per second limit
            duration_sec = size_bytes / 32000.0
            await asyncio.sleep(duration_sec + 0.2)
        except Exception as e:
            self.logger.warning(f"Audio playback duration simulation error: {e}")
