...
class ConversationManager:
    ...
    async def _process_locked(self, text: str, source: str) -> bool:
        ...
        # [Ottimizzazione Concorrenza] Recupero frame e ricerca memorie in parallelo
        frame_task = self._get_latest_frame()
        memory_task = self.memory_manager.search(clean_text, limit=3)
        
        frame, results = await asyncio.gather(frame_task, memory_task)
        
        # FIX: frame.b64 è una @property, non un metodo!
        images = [frame.b64] if (frame and self._is_vision_request(clean_text)) else []

        # 1. Processamento risultati RAG
        memory_context = ""
        try:
            ...
            self._logger.warning(f"RAG processing unexpected error: {e}")
        ...
        augmented_prompt = self._build_prompt(clean_text, ha_context, memory_context, world_context)

        # Timeout dall'oggetto config.llm.timeout (di base accesskey)
        llm_timeout = 60.0
        try:
             llm_timeout = float(self.config.get("llm", {}).get("timeout", 60.0))
        except:
             pass
        ...
        # [NATURAL INTERACTION] Auto-mute rimosso: vogliamo che Marcus possa essere interrotto
        # if self.mic_mute_pub:
        #     from std_msgs.msg import Bool
        #     msg = Bool()
        #     msg.data = True
        #     self.mic_mute_pub.publish(msg)
        #     self._logger.info("Auto-muting mic after AI response.")
    ...
    def _build_prompt(self, user_text: str, ha_context: str, memories: str = "", world_context: str = "") -> str:
