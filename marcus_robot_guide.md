...
* **Controllo Motori, PCNT Hardware & Odometria Reale Fusa:** Il nodo `waveshare_motor_driver` comunica via seriale persistente (`/dev/motor_driver` a 115200 baud) con la board motori ESP32. Il firmware a basso livello esegue un anello chiuso di velocità Feedforward+PI a 50 Hz (`Kp=3.20, Ki=0.22, Kd=0.04`) e frenata rigenerativa **Dynamic Short-Brake** sui MOSFET low-side del ponte TB6612FNG, eliminando torsioni e scivolamenti inerziali all'arresto. I comandi di velocità sono filtrati da un **Profiler S-Curve Jerk Limiter di 2° ordine ($C^1$)** ($j_{max}=25.0\text{ duty/s}^2, a_{max}=5.0\text{ duty/s}$) che elimina il beccheggio e le oscillazioni elastiche dell'albero sensori (LiDAR C1 e OAK-D Lite). I canali encoder sfruttano l'unità hardware ad impulsi (PCNT) con filtro glitch da $2.0\,\mu\text{s}$ e gating hardware di direzione, bypassando in silicio il canale C2 interrotto. L'odometria (`/odom`) calcola gli effettivi spostamenti metrici delle ruote tramite integrazione Runge-Kutta a punto medio di 2° ordine, fusa in tempo reale tramite **filtro complementare ($\alpha=0.88$)** con il giroscopio Z montato sullo chassis ESP32 (con auto-bias tracking stazionario ad EMA e fallback automatico 100% ruote su timeout $>250\text{ ms}$) e Tier 1 Zero-Velocity Lock per annullare qualsiasi deriva da fermo.
...
* **Sicurezza Attiva & Diagnostica Chassis:** Il driver motori monitora costantemente lo stato del movimento per prevenire collisioni e sovraccarichi:
  ...
  * *Sovraccarico elettrico:* se si registra una caduta di tensione della batteria $> 2.0$ V rispetto allo stato di riposo (corrispondente a un picco anomalo di assorbimento).
  Al rilevamento di una di queste condizioni, viene pubblicato uno stato `ERROR` sul topic `/diagnostics` (nodo `motor_stall`). L'orchestratore AI intercetta il diagnostico, attiva l'arresto di emergenza (`emergency_stop()`), cancella istantaneamente il goal di navigazione Nav2 (`nav_client.cancel_navigation()`), avverte l'utente vocalmente con TTS e attiva l'**Amygdala Hijack** con memorizzazione protetta indelebile dell'evento traumatico (`MemoryType.SYSTEM_EVENT`, `lambda_decay=0.0`). Al ripristino nominale, l'Amigdala riarma automaticamente lo stato a `CALM`.
...
* **SLAM e Localizzazione Ibrida Multi-Sensore:** Marcus supporta un'architettura duale flessibile e priva di conflitti TF REP-105:
  * *Modalità Mappatura / SLAM 3D (`--slam`):* Esegue **RTAB-Map** (`rtabmap_slam`) per generare mappe d'ambiente 2D/3D su `/mnt/ssd/rtabmap.db` con fusione duale (`Grid/Sensor: 2` Both, `Reg/Strategy: 2` Visual + ICP) e loop closure DBoW3 (`Kp/DetectorStrategy: 8`, `Rtabmap/LoopThr: 0.20`). Consente l'esportazione immediata della mappa 2D tramite lo script dedicato `./scripts/save_map.sh [nome_mappa]` (salvata in `/mnt/ssd/maps/[nome].yaml` e `.pgm`).
  * *Modalità Navigazione Mappa Statica (`--amcl --map=...` - Opzione A, FM-NAV-030):* In ambienti già mappati, attiva **Nav2 AMCL** (Adaptive Monte Carlo Localization) accoppiato al `nav2_map_server` a 10 Hz su `/scan`. Questa modalità azzera le derive angolari (wheel scrub) e i fallimenti ICP durante curve strette o rotazioni rapide. In questa configurazione RTAB-Map disattiva l'aggiornamento memoria e il broadcast TF (`publish_tf: false`), mantenendo attive la proiezione 2.5D e le memorie visive TRINITY senza generare conflitti sull'albero TF.
* **Odometria Visivo-Inerziale & Robustezza (`fast_flow_vo_cpp` & `localization_fuser_node`):** Fusione locale ad alta frequenza (30-50 Hz) che unisce la dinamica lineare degli encoder ruote (con polarità normalizzata a doppio encoder decrementale) con l'heading immediato del giroscopio BMI270 a 200 Hz (orientamento conforme a REP-103 con inversione asse Y telecamera DepthAI: $\omega_{z,ros} = -\omega_{y,cam}$) e compensazione dinamica del pitch sag a $8^\circ$. Garantisce deriva nulla da fermo e coerenza bidirezionale con lo scan-matching 360° del LiDAR RPLIDAR C1 su RTAB-Map, eliminando qualsiasi distorsione della mappa. Non usa `robot_localization` EKF per prevenire blocchi o deadlock storici da flag IMU non conformi (`FM-NAV-004`).
...
* **Gestione Alimentazione, Battery Management System (BMS) & Voltage Feed-Forward:**
  Marcus dispone di un'architettura di alimentazione a doppio binario (*Power Path OR-ing* tramite diodi ideali). La tensione e la corrente del bus `DC_IN` (proveniente direttamente dalla batteria LiPo 3S2P senza step-down) vengono campionate con precisione dal sensore digitale **Texas Instruments INA219 (I2C 0x42, shunt 10 mOhm)** a bordo della scheda ESP32 Waveshare. Il nodo `battery_manager_node.py` gestisce:
...
* **Interfaccia Seriale & LED ReSpeaker Lite (`respeaker_interface_node.py` - FM-VUI-024):** Comunica via porta persistente deterministica `/dev/respeaker` (con fallback automatico su `/dev/serial/by-id/*` e `/dev/ttyACM*`) con il microcontrollore Seeed XIAO ESP32-S3 per il controllo hardware dei LED RGB di stato (`IDLE`, `LISTENING`, `THINKING`, `ERROR`) ed il watchdog heartbeat.
...
L'HAT Hardware Raspberry Pi AI HAT+ ospita una **Hailo-10 NPU** da 40 TOPS collegata via PCIe Gen 3. Gestita nativamente tramite il driver C++ High-Performance `hailo_bridge_node_cpp` (`<hailo/infer_model.hpp>`, HailoRT 5.1 InferModel API), azzera l'overhead del GIL Python (carico CPU <15% su Core 2-3) e supporta Lazy Publishing per la trasmissione di immagini annote. Gestisce il binding dinamico di tutti gli stream di input per HEF multi-rete congiunti (`marcus_unified.hef`), la decodifica anchor-free YOLOv8 DFL a 3 scale (80x80, 40x40, 20x20) e l'NMS veloce direttamente in C++. Marcus la sfrutta per eseguire reti neurali pesanti localmente a FPS elevati:

1. **YOLOv8-seg / YOLOv8 Detection:** Rilevamento in tempo reale di 80 classi di oggetti (persone, sedie, tavoli, bottiglie) con etichette in italiano e pubblicazione sincrona su `/hailo/detections` e `/hailo/semantic_objects` per la costmap 2.5D di Nav2.
...
7. **Requisiti Energetici Hardware (SPEC-06 / FM-PWR-002):** A pieno carico computazionale (NPU + OAK-D + SSD NVMe), il Raspberry Pi 5 richiede **5.10V - 5.25V @ 5A** continui (25W). L'alimentazione deve provenire da step-down sincrono tarato a 5.20V-5.25V collegato direttamente ai pin GPIO 2/4 (5V) e 6/9 (GND) con conduttori 18-20 AWG, per prevenire brownout del PMIC DA9091 causati dalla caduta resistiva dei cavi USB sottili.
...
5. **Project Autopoiesis (Data Mining con Idle Gating e Auto-Miglioramento Continuo):**
   * *Data Mining Misurato (`MarcusDataMiner`):* Raccolta passiva di telemetria a 0.1 Hz (cross-track error, jitter angolare, recovery count, RAM/CPU) con **idle gating categorico: a navigazione spenta e robot fermo, la raccolta è completamente sospesa**, azzerando usura SSD e carico CPU. Buffer limitato a 60 campioni (10 minuti di moto effettivo) e flush transazionale batch su SQLite WAL su SSD.
   * *Selezione Autonoma Quotidiana (Daily Focus):* Durante il `Nightly Dream Service` (ore 03:00), Marcus sceglie autonomamente il tema ingegneristico più critico incrociando i colli di bottiglia telemetrici reali sul campo con i massimi RPN del database FMEA (`dfmea.yaml`), notificando la scelta su `docs/evolution/daily_focus_notifications.md` e sul topic `/robot_ai/notifications` (senza dipendenze da Home Assistant).
   * *Pipeline Multi-Modello Specializzata:* L'Orchestratore **Gemini 3.1 Pro** sviscera a fondo l'architettura di notte e scompone il problema in micro-task sequenziali; il Coder **Gemini 3.8 Flash** implementa e collauda il codice in sandbox con la massima efficienza di token.

---

## 🚀 Guida Rapida Operativa: Comandi di Avvio & Sincronizzazione

### 1. Modalità Operative di Avvio sul Robot (`restart_hailo.sh` / `restart.sh`)

| Obiettivo | Comando da Terminale (su Pi 5) | Note Operative & Comportamento |
| :--- | :--- | :--- |
| **Crea Mappa (SLAM)** | `./restart_hailo.sh --slam` *(o `./restart.sh`)* | Avvia RTAB-Map SLAM 3D/2D con ICP a 360° e loop closure DBoW3. Autorità TF `map -> odom` su RTAB-Map. Salva il grafo su `/mnt/ssd/rtabmap.db`. |
| **Salva Mappa 2D** | `./scripts/save_map.sh [nome_mappa]` | Esporta `/map` in `/mnt/ssd/maps/<nome_mappa>.yaml` e `.pgm` via `map_saver_cli` di Nav2. |
| **Naviga (Mappa Statica)** | `./restart_hailo.sh --amcl --map=/mnt/ssd/maps/<nome>.yaml` | **Opzione A (FM-NAV-030):** Attiva AMCL (10 Hz) e Map Server per localizzazione su mappa nota. RTAB-Map disattiva `publish_tf` e salvataggio incrementale, mantenendo attive la sicurezza 2.5D, prevenzione scale e memorie TRINITY senza conflitti TF REP-105. |

### 2. Sincronizzazione & Auto-Aggiornamento del Codice (`sync_marcus.sh`)

La sincronizzazione tra macchina di sviluppo (PC) e robot (Raspberry Pi 5) è gestita in sicurezza dallo script `./sync_marcus.sh`:
* **SSH Multiplexing (ControlMaster):** Una sola richiesta di password SSH per tutta la sessione.
* **Anti-Flattening Guard (Back-Sync Robot ➔ PC):** Recupera preventivamente dal robot le abilità generate dall'IA (`skills/`), i log, i registri evolutivi (`docs/evolution/`) e la DFMEA (`fmea/dfmea.yaml`), impedendo che una modifica da PC cancelli il lavoro autonomo di Marcus.
* **Forward-Sync Protetto (PC ➔ Robot):** Utilizza il flag `-u` (`--update`) per sincronizzare solo i file più recenti.
* **Python Hot-Swap Istantaneo:** Sincronizza `nodes/`, `robot_ai/` e `launch/` direttamente nella cartella di installazione (`install/robopy_controller/lib/python3.x/site-packages/`), permettendo di testare le modifiche senza ricompilare il workspace.
