...
* **Controllo Motori, PCNT Hardware & Odometria Reale:** Il nodo `waveshare_motor_driver` comunica via seriale persistente (`/dev/motor_driver` a 115200 baud) con la board motori ESP32. Il firmware a basso livello esegue un anello chiuso di velocità Feedforward+PI a 50 Hz (`Kp=3.20, Ki=0.22, Kd=0.04`) e frenata rigenerativa **Dynamic Short-Brake** sui MOSFET low-side del ponte TB6612FNG, eliminando torsioni e scivolamenti inerziali all'arresto. I canali encoder sfruttano l'unità hardware ad impulsi (PCNT) con filtro glitch da $2.0\,\mu\text{s}$ e gating hardware di direzione, bypassando in silicio il canale C2 interrotto. L'odometria (`/odom`) calcola gli effettivi spostamenti metrici delle ruote tramite integrazione Runge-Kutta a punto medio di 2° ordine (abbandonando l'odometria teorica da `/cmd_vel`) con Tier 1 Zero-Velocity Lock per annullare qualsiasi deriva da fermo.
