...
* **Odometria Visivo-Inerziale Nativa C++ (`fast_flow_vo_cpp`):** La stima del movimento visivo-inerziale a 30 Hz è affidata al nodo nativo C++ `fast_flow_vo_cpp`. Sfrutta l'offloading hardware della VPU MyriadX dell'OAK-D Lite (`dai::node::FeatureTracker` + KLT Optical Flow) e l'integrazione del giroscopio IMU 200 Hz Bosch BMI270. È l'unica autorità per la trasformazione TF `odom -> base_link`, sopprimendo lo slittamento delle ruote e consentendo una precisione rotazionale sul posto del 99.6% (misurati 358.4° su 360.0°).
