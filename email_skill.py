...
class EmailSkill(BaseSkill):
    ...
    async def _fetch_emails_task(self, intent: str) -> List[Dict]:
        ...
        try:
            ...
            # search_data[0] è bytes: b'1 2 3' oppure b'' se nessun risultato
            raw_ids_str = search_data[0].decode() if search_data[0] else ""
            all_ids     = raw_ids_str.split()              # lista di stringhe
            msg_ids     = all_ids[-self.max_emails:]       # ultimi N

            logger.info(
                f"IMAP: trovati {len(all_ids)} messaggi, "
                f"processo gli ultimi {len(msg_ids)}"
            )
    ...
    async def execute(
        ...
        try:
            ...
            try:
                ...
                err_msg = str(e)
                speak_text = "Non riesco ad accedere alla tua email. "
                
                # Check for various Google/IMAP "App Password" required strings
                if "Application-specific password required" in err_msg or "support.google.com/accounts/answer/185833" in err_msg:
                    speak_text += "Gmail richiede una password specifica per le app. Generala nelle tue impostazioni di sicurezza Google."
                else:
                    speak_text += "Controlla le credenziali o abilita la password per app."
                
                yield SkillResult.failure_result(
                    f"Accesso email negato: {err_msg}",
                    error_code=SkillErrorCode.PERMISSION_DENIED,
                    speak=speak_text
                )
                return
