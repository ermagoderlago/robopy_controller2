...
> **Priorità RPN:** 560 (CRITICAL) | **Stato:** PLANNED | **Dominio:** Navigazione & Visione
...
### Modulo SW: `semantic_costmap_injector` (Node ROS 2 Python)
...
- **Algoritmo (Depth-Gradient Hole Raycasting):**
  1. Calcola il piano del pavimento locale tramite fit RANSAC sui punti $Z \in [0.5, 1.5]$ metri.
  2. Identifica celle dove la profondità misurata eccede di oltre $\Delta Z > 15\text{ cm}$ rispetto al piano terra teorico.
  3. Iniettare un ostacolo letale (`254` cost) nella costmap locale a livello del bordo del dislivello.

### Modulo HW: Integrazione Cliff Sensors (ESP32)
...
- [ ] **Task 1 (SW):** Creare lo script `negative_obstacle_detector_node.py` sotto `robopy_controller/nodes/`.
- [ ] **Task 2 (SW):** Sottoscrivere `/oak/stereo/image_raw` e pubblicare ostacoli letali su `/costmap/costmap_updates`.
- [ ] **Task 3 (HW):** Configurare la routine di interrupt GPIO sull'ESP32 per il feedback dai 3 Cliff Sensor.
- [ ] **Task 4 (Test):** Eseguire test con imbracatura di sicurezza sul bordo dello scalino e registrare il log di intervento.

---

## 4. Criteri di Accettazione & Validazione

- **Accettazione SW:** Il robot rileva uno scalino di 10cm da almeno 80cm di distanza e arresta la traiettoria prima del bordo.
