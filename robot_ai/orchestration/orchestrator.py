...
class AIOrchestrator(Node):
    def __init__(self):
        ...
        self.tts_speaking_pub = self.create_publisher(Bool, '/ai/tts/speaking', 10)
    ...
    def _setup_ros_interfaces(self):
        ...
        self.event_bus.subscribe(EventType.SPEECH_STARTED,   self._on_tts_started) # Re-use the same True signal
    ...
    def _audio_chunk_callback(self, msg: AudioData):
        if self._shutdown_flag:
            return
        # Routing nativo zero-latenza (Gemini Native Audio) al nodo VUI!
    ...
    def _on_tts_started(self, event):
        self.ai_logger.info("🎤 [ORCHESTRATOR] TTS Started signal")
        msg = Bool()
        msg.data = True
        self.tts_speaking_pub.publish(msg)

    def _on_tts_completed(self, event):
        self.ai_logger.info("🎤 [ORCHESTRATOR] TTS Completed signal")
        msg = Bool()
        msg.data = False
        self.tts_speaking_pub.publish(msg)

    def _on_live_turn_complete(self, event):
        self.ai_logger.info("🎤 [ORCHESTRATOR] Live Turn Complete signal")
        msg = Bool()
        msg.data = False
        self.tts_speaking_pub.publish(msg)
    ...
    async def _async_init(self):
        try:
            ...
            # Saluto nativo via Live API (Gemini Native Audio)
            # Questo evita il problema delle credenziali Google TTS mancanti
            if self.llm_service:
                self.ai_logger.info("Marcus starting greeting (Non-blocking, Barge-in enabled)...")
                # Segnaliamo attività vocale manualmente per il saluto (prima che arrivi il primo chunk)
                msg = Bool()
                msg.data = True
                self.tts_speaking_pub.publish(msg)
                
                asyncio.create_task(self.llm_service.generate_live(chosen_greeting))
            else:
                self.ai_logger.warning("LLM Service not available for greeting, falling back to basic TTS.")
                asyncio.create_task(self.tts_service.speak(chosen_greeting))
        except Exception as e:
            self.ai_logger.error(f"Init failed: {e}")
            # Reset di sicurezza in caso di crash pre-vocalizzazione
            msg = Bool()
            msg.data = False
            self.tts_speaking_pub.publish(msg)
