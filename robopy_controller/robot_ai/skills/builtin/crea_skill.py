...
=====================================================
Skill che permette a Severus di generare nuove skill usando la pipeline skill_generator.
"""
...
class CreaSkill(BaseSkill):
    ...
    async def execute(self, text: str, context: Dict[str, Any] = None) -> SkillResult:
        ...
        """
        # 1. Estrazione parametri base (Severus LLM dovrebbe averli già estratti via Tool Calling if enabled,
        # ma se viene chiamato via match() dobbiamo estrarli noi o chiedere all'utente).
        
        # Se siamo in Tool Calling, Severus passerà i parametri. 
        # Se siamo in match(), dobbiamo dedurre nome e descrizione.
        
        # Nota: In Severus AI, le skill builtin sono spesso usate come Tool nel LLMService.
        # Se Severus parla con Gemini Live API, userà il tool definition.
        
        yield SkillResult(True, "Inizio generazione...", "Certamente. Sto analizzando la tua richiesta per creare una nuova capacità.")
        
        try:
            # Funzione helper che Severus (l'LLM) userà per generare il codice nel prompt della pipeline
    ...
    def to_function_declaration(self) -> Dict[str, Any]:
        ...
        return {
            "name": "crea_skill",
            "description": "Genera una nuova skill (capacità) per il robot Severus. Usa questo tool quando l'utente ti chiede di imparare a fare qualcosa di nuovo o di creare una nuova funzionalità.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
