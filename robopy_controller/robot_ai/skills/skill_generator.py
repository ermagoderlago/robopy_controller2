...
class SkillGeneratorPipeline:
    ...
    def __init__(
        ...
        if workspace_root is None:
            # Risali dalla posizione di questo file (skills/) al package root (robopy_controller/)
            # skill_generator.py → skills/ → robot_ai/ → robopy_controller/
...
"""
    ...
    def update_ai_context_for_skill(self, request: SkillRequest) -> bool:
        """
        Aggiunge la nuova skill alla sezione delle skill in ai_context.md.

        Questo file viene letto da ingest_knowledge.py per aggiornare la RAG.
        Aggiornarlo ora garantisce che Marcus sappia della skill al prossimo ciclo.

        Returns:
            True se aggiornato con successo
        """
        try:
            ai_context_path = self.workspace_root.parent / "ai_context.md"
            if not ai_context_path.exists():
                logger.warning(f"ai_context.md non trovato: {ai_context_path}")
                return False

            content = ai_context_path.read_text(encoding='utf-8')

            # Evita duplicati
            if request.snake_name in content:
                logger.info(f"ai_context.md già contiene {request.snake_name}, skip.")
                return True

            new_entry = (
                f"- `robopy_controller/robot_ai/skills/active/{request.snake_name}.py`\r\n"
                f"  [AUTO] {request.description}\r\n"
                f"  Keywords: {', '.join(request.test_utterances[:3])}\r\n"
            )

            # Inserisci dopo l'ultima riga della sezione skill builtin
            anchor = "- `robopy_controller/robot_ai/skills/builtin/email_skill.py`"
            if anchor in content:
                # Trova la fine della riga anchor e inserisci subito dopo
                idx = content.index(anchor) + len(anchor)
                # Trova il fine riga
                nl_idx = content.find('\n', idx)
                if nl_idx == -1:
                    nl_idx = len(content)
                content = content[:nl_idx + 1] + new_entry + content[nl_idx + 1:]
            else:
                # Fallback: aggiungi in fondo
                content += f"\r\n## Skill generate automaticamente\r\n{new_entry}"

            ai_context_path.write_text(content, encoding='utf-8')
            logger.info(f"ai_context.md aggiornato con skill: {request.name}")
            return True

        except Exception as e:
            logger.error(f"Errore aggiornamento ai_context.md: {e}")
            return False

    async def add_skill_to_rag(
        self,
        request: SkillRequest,
        memory_manager=None,
    ) -> bool:
        """
        Aggiunge la descrizione della skill alla RAG di Marcus in runtime.

        Non richiede ingest_knowledge.py (SSH). Usa il memory_manager
        dell'orchestratore se disponibile per un update immediato.

        Returns:
            True se aggiunto con successo
        """
        rag_text = (
            f"Nuova skill disponibile: {request.name}. "
            f"{request.description}. "
            f"Puoi usarla dicendo: {', '.join(request.test_utterances)}. "
            f"Capability: {', '.join(request.capabilities) if request.capabilities else 'nessuna'}."
        )
        ...
        if memory_manager is not None:
            try:
                await memory_manager.add(
                    content=rag_text,
                    metadata={"type": "skill_registration", "skill": request.name},
                    importance=0.9,
                )
                logger.info(f"Skill '{request.name}' aggiunta alla RAG runtime")
                return True
            except Exception as e:
                logger.warning(f"RAG runtime update fallito: {e}")

        logger.info(f"RAG update manuale richiesto per '{request.name}'. Testo: {rag_text}")
        return False

    def write_skill_creation_log(
        self,
        request: SkillRequest,
        step_logs: list,
        success: bool,
        ...
        """
        Scrive il log completo di creazione skill su file dedicato.

        Il log include tutti gli step, errori e azioni intraprese.
        Permette l'intervento manuale post-mortem in caso di fallimento.

        Returns:
            Path al file di log creato
        """
        timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        log_file = self.logs_dir / f"SKILL_LOG_{request.snake_name}_{timestamp}.txt"
        ...
        lines = [
            ...
            f"File:          active/{request.snake_name}.py",
            f"Risultato:     {status}",
            f"Timestamp:     {datetime.utcnow().isoformat()}Z",
            f"Richiesta:     {original_text!r}",
            "",
            "--- PARAMETRI ESTRATTI ---",
            f"Nome:          {request.name}",
            f"Descrizione:   {request.description}",
            f"Capabilities:  {request.capabilities}",
            f"Topic SUB:     {request.topics_sub}",
            f"Topic PUB:     {request.topics_pub}",
            f"Utterances:    {request.test_utterances}",
            "",
            "--- STEP LOG ---",
        ]

        for ts, msg in step_logs:
            lines.append(f"[{ts}] {msg}")

        lines += [
            "",
            "--- FILE GENERATI ---",
            f"Active:        robot_ai/skills/active/{request.snake_name}.py",
            f"Log generator: logs/LOG_{request.snake_name}_*.txt",
            f"Questo log:    logs/{log_file.name}",
        ]
        ...
        if not success:
            lines += [
                ...
                f"1. Controllare: logs/LOG_{request.snake_name}_*.txt",
                "2. Verificare topic in files_topic.md",
                "3. Riformulare la richiesta con piu' contesto",
                f"4. File fallito in: robot_ai/skills/failed/{request.snake_name}.py",
            ]

        log_file.write_text("\n".join(lines), encoding='utf-8')
        logger.info(f"Skill creation log: {log_file.name}")
        return log_file
...
"""
    ...
    async def run_full_pipeline(
        ...
        Args:
            ...
            step_callback: Async callable opzionale(messaggio: str) -> None
                           chiamato ad ogni step per feedback in tempo reale

        Returns:
            SkillGenerationResult finale
        """
        async def _notify(msg: str):
            """Invia notifica step se callback disponibile."""
            logger.info(f"[PIPELINE STEP] {msg}")
            if step_callback is not None:
                try:
                    await step_callback(msg)
                except Exception:
                    pass  # Non bloccare la pipeline per errori nel callback
        ...
        for iteration in range(1, MAX_ITERATIONS + 1):
            # Prepara prompt
            if iteration == 1:
                await _notify(f"Preparo il prompt di generazione per '{request.name}'...")
                prompt = self.prepare_prompt(request)
            else:
                await _notify(
                    f"Iterazione {iteration}/{MAX_ITERATIONS}: "
                    f"auto-repair in corso dopo {len(errors)} errori..."
                )
                prompt = self.prepare_repair_prompt(request, last_code, errors)

            # Chiama Gemini per il codice
            await _notify(
                f"Iterazione {iteration}/{MAX_ITERATIONS}: "
                f"Gemini sta scrivendo il codice della skill..."
            )
            try:
                raw_code = await code_provider(prompt)
            except Exception as e:
                logger.error(f"Code provider fallito iter {iteration}: {e}")
                await _notify(f"❌ Iterazione {iteration}: generazione codice fallita: {e}")
                continue

            last_code = raw_code

            # Notifica dimensione del codice generato
            code_len = len(raw_code)
            await _notify(
                f"Iterazione {iteration}: codice generato ({code_len} chars). "
                f"Avvio Quality Gate..."
            )

            # Processa (AST + Smoke + Sandbox)
            result = await self.process_generated_code(
                request, raw_code, iteration
            )

            # Notifica esito Quality Gate
            if result.success:
                await _notify(f"✅ Iterazione {iteration}: Quality Gate superato!")
                return result
            else:
                errors = []
                if result.failure_report:
                    errors = result.failure_report.split("\n")
                error_summary = "; ".join(e for e in errors[:3] if e.strip())
                await _notify(
                    f"❌ Iterazione {iteration}/{MAX_ITERATIONS}: "
                    f"Quality Gate fallito. Errori: {error_summary}"
                )
                last_result = result
