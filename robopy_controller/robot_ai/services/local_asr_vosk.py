...
class VoskASRManager:
    ...
    """
    def __init__(self, model_path="/mnt/ssd/robopy_controller_host/models/vosk-model-small-it-0.22", sample_rate=16000, on_text_cb=None):
    ...
    def force_flush(self):
        ...
        if not self.is_active() or not self.recognizer:
            return
        res = self.recognizer.FinalResult()
        text = json.loads(res).get("text", "").strip()
        if text:
            self._handle_transcription(text)
        
        # Reset recognizer state for the next phrase
        self.recognizer = vosk.KaldiRecognizer(self.model, self.sample_rate)
