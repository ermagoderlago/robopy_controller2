...
class OakDv3Node(Node):
    ...
    def _build_pipeline_v3(self) -> bool:
        try:
            ...
            low_out = cam.requestOutput((self.low_w, self.low_h), type=dai.ImgFrame.Type.BGR888p)
            preview_out = cam.requestOutput((self.nn_w, self.nn_h), type=dai.ImgFrame.Type.BGR888p)
            ...
            # 5. Create Queues
            self.q_low = low_out.createOutputQueue(maxSize=4, blocking=False)
            self.q_preview = preview_out.createOutputQueue(maxSize=4, blocking=False)
            ...
            if self.enable_nn and det:
                self.q_nn = det.out.createOutputQueue(maxSize=4, blocking=False)
