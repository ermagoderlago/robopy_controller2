...
class ReSpeakerVUINode(Node):
    ...
    def _process_vad_frame(self, frame_int16: np.ndarray) -> None:
        ...
        # [DSP-HOT] Calcolo RMS per noise gate pre-VAD
    ...
    def _audio_input_callback(self, in_data, frame_count, time_info, status):
        ...
        try:
            ...
            if self._cfg_diag_mode:
                ...
                self.get_logger().info(
                    f"🎤 [MIC] Volume: L_RMS={rms_l:.1f} | R_RMS={rms_r:.1f} | FORCED_BOOSTED={rms_boosted:.1f} "
                    f" (Gain: {stt_gain_to_use}x)")
            ...
            # ---------------------------------------------------------- #
            # [v5.6] Controllo TTS — NUOVO: non blocca più l'audio pipeline.
            ...
            # Transizione False→True: registra timestamp di inizio TTS
            ...
            # Transizione True→False: TTS appena terminato — reset barge-in
